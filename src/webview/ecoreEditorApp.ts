...
export class EcoreEditorApp {
  ...
  private setupEventListeners(): void {
    ...
    document
      ...
      ?.addEventListener('click', () => this.treeView.expandAll());
    document
      .getElementById('collapse-all-btn')
      ?.addEventListener('click', () => this.treeView.collapseAll());
  ...
  private onTreeSelectionChanged(
    element: any,
    focusNameField: boolean = false,
  ): void {
    this.propertiesPanel.showProperties(element, focusNameField);
  ...
  private onPropertyChanged(element: any, property: string, value: any): void {
    ...
    try {
      ...
      if (property === 'name' && !this.propertiesPanel.isUpdating) {
        // Re-show properties to update the header only if not updating from within properties panel
        ...
        if (selectionInfo) {
          const type = this.getElementType(element);
          selectionInfo.textContent = `${type}: ${value || 'unnamed'}`;
        }
      } else if (property === 'name' && this.propertiesPanel.isUpdating) {
        // Just update the status bar without rebuilding properties
        const selectionInfo = document.getElementById('selection-info');
        if (selectionInfo) {
          const type = this.getElementType(element);
          selectionInfo.textContent = `${type}: ${value || 'unnamed'}`;
      ...
      // For all property changes (including checkboxes, dropdowns), update the tree
      // This includes: abstract, interface, containment, multiplicity, etc.
      if (
        property === 'eSuperType' ||
